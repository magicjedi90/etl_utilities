...
class BulkLoader:
    ...
    def bulk_insert(self, df: pd.DataFrame, batch_size: int = 1000) -> None:
        ...
        try:
            ...
            self.logger.info(f'Inserted {len(data)} rows into {self.schema_name}.{self.table_name}.')
        except Exception as e:
            cursor.rollback()
            self.logger.error(f'Error inserting data into {self.schema_name}.{self.table_name}: {str(e)}')
